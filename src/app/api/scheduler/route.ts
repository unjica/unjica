    ...
    handler: async () => {
      ...
      try {
        ...
        if (existingArticles.length > 0) {
          console.log(`Art digest already generated today. Found ${existingArticles.length} articles.`);
          console.log(`Latest article: (${existingArticles[0].id}): ${existingArticles[0].title}`);
          return true;
        }
        
        // Add a small random delay to reduce chance of concurrent executions
        const randomDelay = Math.floor(Math.random() * 5000); // Random delay up to 5 seconds
        console.log(`Adding random delay of ${randomDelay}ms to reduce concurrent executions`);
        await new Promise(resolve => setTimeout(resolve, randomDelay));
        
        // Check again after the delay to see if another process has created an article
        const articlesAfterDelay = await prisma.generatedArticle.findMany({
          where: {
            publishedAt: {
              gte: today,
              lt: tomorrow
        ...
        if (articlesAfterDelay.length > 0) {
          console.log(`Art digest was generated by another process during delay. Found ${articlesAfterDelay.length} articles.`);
          console.log(`Latest article: (${articlesAfterDelay[0].id}): ${articlesAfterDelay[0].title}`);
          return true;
        }
